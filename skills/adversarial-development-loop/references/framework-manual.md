...
> **何时读**:采纳/扩展本框架、新增领域包种子、同步安装副本、**或修改 SKILL.md 内核**(§8 维护纪律)时。日常对抗循环执行**不需要**本文件——内核在 `SKILL.md`。
...
## 8. 内核维护纪律(改 SKILL.md 时必读;iter 538 从内核迁入,规则不变)

内核曾从 ~180 行涨到 417 行全程没淘汰一条——给项目定了 Pattern Index"加新时淘汰旧"的纪律,自己反而没有,是双重标准。**改 SKILL.md 加新规则/核查项/案例时,必做三步:**

1. **grep 全文找被新条目覆盖、替代、稀释的旧条目**。有 → 合并(留权威定义处,其余改"见 X")或淘汰。
2. **标权威 vs 引用**:每个概念只在一处展开,其余位置写"见 X"(先例:Step 4"唯一权威定义"、Step 5"R3 引用此处")。
3. **加 iter 案例时问:这个案例佐证的是新条目还是已有条目?** 佐证已有条目 → 加到已有条目旁,不开新条目。

**判定指标**:一次修改让内核净增 >15 行,大概率没执行本纪律——回头 grep 重叠。
